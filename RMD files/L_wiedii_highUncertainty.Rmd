...
#To demonstrate the use of the package occUncertain we used a a small wild cat in Ecuador and Colombia, the margay (*Leopardus wiedii*). The data set of 126 verified georeferenced occurrence records, includes high uncertainty above 3000 meters and NA uncertainty records. The occurrences dataset are from the Global Biodiversity Information Facility (19 May 2020) GBIF Occurrence Download https://doi.org/10.15468/dl.vk997h

load("../data/L_wiedii_highUnc.RData")

L_wiedii_highUnc_observed <-
  dplyr::select(L_wiedii_highUnc,
    decimalLatitude,
    decimalLongitude,
    species)
write.csv(L_wiedii_highUnc_observed, "../Results/L_wiedii_highUnc_observed.csv")

data(land)
observed.IUCN <- IUCN.eval(L_wiedii_highUnc_observed, country_map = land, exclude.area = T, write_results = F)
observed.EOO <- observed.IUCN$EOO
observed.AOO <- observed.IUCN$AOO
...
plot(L_wiedii_highUnc$longitude, L_wiedii_highUnc$latitude, pch = 19, col = "black", xlab = "Longitude", ylab = "Latitude", 
...
```{r L_wiedii_high_naMean geographic metrics}
library(ConR)
...
(
  observed.EOO - mean(L_wiedii_high_naMean_geo_range_100$naMean_EOO)
) / sd(L_wiedii_high_naMean_geo_range_100$naMean_EOO)
```
