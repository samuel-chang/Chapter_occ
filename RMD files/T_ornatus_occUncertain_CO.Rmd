...
#To demonstrate the use of the package occUncertain we used a set of four different datasets. This dataset is for 65 georeferenced occurrence records in Colombia for the spectacled bear (*Tremarctos ornatus*). These occurrences dataset are from the Global Biodiversity Information Facility (GBIF) and are included as part of the occUncertain package.

## 3.1 Load dataset 
```{r dataset, eval = FALSE}
save(T_ornatus_CO, file = "~/GitHub/ENS Master/Chapter1/Chapter_uno/data/T_ornatus_CO.RData")
...
mapPoints_T_ornatus <- gmap(map) +
+ geom_point(aes(x = T_ornatus$longitude, y = T_ornatus$latitude, data = T_ornatus, alpha = .5))
plot(T_ornatus$longitude, T_ornatus$latitude)
points(T_ornatus_random$lon_random, T_ornatus_random$lat_random, col = "red", pch = 21)
```
----
...
```{r land metrics}
T_ornatus_CO_naMean_geo_range_10 <-
