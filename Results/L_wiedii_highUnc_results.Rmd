...
observed <-
  dplyr::select(L_wiedii_highUnc,
    ...
    decimalLongitude,
    species)
write.csv(observed, "../Results/observed.csv")

observed.IUCN <- IUCN.eval(observed, exclude.area = T, write_results = F)
...
hist(L_wiedii_high_100georanges$naMean_EOO,
  main = "L. wiediee EOO with mean ucertainty values for NA cells")
abline(v = observed.EOO,
  col = "red")

#AOO_100
(observed.IUCN$AOO - mean(L_wiedii_high_100georanges$naMean_AOO)
) / sd(L_wiedii_high_100georanges$naMean_AOO)

#1000 iteration is -0.4048546
#EOO_1000
(observed.EOO$EOO - mean(L_wiedii_high_1000georanges$naMean_EOO)
) / sd(L_wiedii_high_1000georanges$naMean_EOO)

hist(L_wiedii_high_1000georanges$naMean_EOO,
  main = "L. wiedii extent of occurrence, with mean ucertainty values for NA cells")
abline(v = observed.EOO$EOO,
  col = "red")

#AOO_1000
(observed.IUCN$AOO - mean(L_wiedii_high_1000georanges$naMean_AOO)
) / sd(L_wiedii_high_1000georanges$naMean_AOO)
```
